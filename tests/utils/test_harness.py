...
@functions.bind('test/worker', specs=tasks.value_specs())
...
class TestHarness:
        ...
        for state_mutation in state_mutations:
            ...
            if state_mutation.mutation_type == state_mutation.MODIFY:
                next(filter(lambda x: x.state_name == state_mutation.state_name,
                            state)).state_value.CopyFrom(state_mutation.state_value)
            elif state_mutation.mutation_type == state_mutation.DELETE:
                pass # flink already zero's out the state value for us
            else:
    ...
    def _try_extract_egress(self, invocation_result):
        ...
        kafka_producer_record.ParseFromString(invocation_result.outgoing_egresses[0].argument.value)
    ...
    def _run_flink_loop(self, message_arg, target: Address, caller=None, egress_result=None):
        ...
        if isinstance(message_arg, TypedValue): # function calling function
            ...
            flink_type = flink_value_type_for(message_arg) # ingress protobuf needs to to be wrapped into a TypedValue
        ...
        for outgoing_message in outgoing_messages:
            egress_value = self._run_flink_loop(message_arg=outgoing_message.argument, target=outgoing_message.target,
